...
data "aws_subnet" "input" {
  ...
  lifecycle {
    ...
    postcondition {
      ...
      error_message = <<-EOT
        The following subnet is not marked as privaite:
...
locals {
  security_group_rule_ids = data.aws_vpc_security_group_rules.input.ids != null ? data.aws_vpc_security_group_rules.input.ids : []
}
...
data "aws_vpc_security_group_rule" "input" {
  for_each               = toset(local.security_group_rule_ids)
